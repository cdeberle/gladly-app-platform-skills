...
Understanding what data is available in each template type is critical for implementation.

### Authentication Headers Context

Templates in `authentication/headers/` have access to:
...
```jsonc
{
    "integration": {
        ...
        "configuration": {},
        // Secrets needed for authenticating against the external system
        "secrets": {}
    }
}
```

### Request Signing Context

Templates in `authentication/request_signing/` have access to:
...
```jsonc
{
    ...
    // HTTP request information used for creating the signature
    "request": {
        // The request URL as generated by request_url.gtpl
        "url": "",
        // The HTTP method (GET, POST, etc.)
        "method": "",
        // Headers from authentication/headers/ templates
        "headers": {
...
### OAuth Authorization Code Context

**auth_code/request_url.gtpl** has access to:

```jsonc
{
    "correlationId": "123456",  // Use for state parameter
    "integration": {
        "configuration": {
...
**access_token/request_url.gtpl and request_body.gtpl** have access to:
...
```jsonc
{
    ...
    "oauth": {
        ...
        "request": {
            "url": "https://gladly.com/auth-code?code=secret&state=xxx",
            "method": "GET",
            "headers": {
        ...
        "state": "state from redirect URI (must match correlationId)"
    }
}
```

**refresh_token/request_url.gtpl and request_body.gtpl** have access to:
...
### OAuth Client Credentials Context
...
```jsonc
{
    "integration": {
        "configuration": {
            "client_id": "OAuth client_id"  // if client was issued a secret
        },
        "secrets": {
            "client_secret": "OAuth client_secret"  // if client was issued a secret
        }
    }
}
```

### OAuth Password Grant Context
